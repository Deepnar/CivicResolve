...
export const OLLAMA_CHAT_MODEL = process.env.OLLAMA_CHAT_MODEL || 'deepseek-v4-flash'
export const OLLAMA_VISION_MODEL = process.env.OLLAMA_VISION_MODEL || 'mimo-v2.5'
...
export async function ollamaGenerate(opts: OllamaGenerateOptions): Promise<string> {
  if (OPENCODE_API_KEY) return opencodeGenerate(opts)
...
async function opencodeGenerate(opts: OllamaGenerateOptions): Promise<string> {
  ...
  if (opts.images?.length && messages.length) {
    ...
    last.content = [
      { type: 'image_url', image_url: { url: `data:image/jpeg;base64,${opts.images[0]}` } },
      { type: 'text', text },
    ]
  }
...
// Ollama fallback (used only when OPENCODE_API_KEY is not configured).
async function ollamaGenerateFallback(opts: OllamaGenerateOptions): Promise<string> {
  const body: Record<string, unknown> = {
    model: opts.model || (opts.images?.length ? 'llama3.2-vision:11b' : 'qwen2.5:3b'),
    stream: false,
    options: {},
  }
